...
import type { Pattern, ConstrainablePoint } from '../pattern';
import { deletePoint, deletePath, deletePiece, deleteSeam, deleteText } from '../utils/patternMutations';
...
export function selectionPointIds(p: Pattern, sel: Selection): Set<string> {
  const ids = new Set(sel.get('point'));
  const pathSet = sel.get('path');
  if (pathSet.size) {
...
function applyAffine(
  ...
  if (movable.size === 0 && sel.get('piece').size === 0) return p;
...
export function selectionDelete(p: Pattern, sel: Selection): Pattern {
  let next = p;
  for (const id of sel.get('seam')) next = deleteSeam(next, id);
  for (const id of sel.get('text')) next = deleteText(next, id);
  for (const id of sel.get('piece')) next = deletePiece(next, id);
  for (const id of sel.get('path')) next = deletePath(next, id);
  for (const id of sel.get('point')) next = deletePoint(next, id);
  return next;
}
